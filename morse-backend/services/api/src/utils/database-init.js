...
async function checkTablesExist() {
  ...
  try {
    const result = await client.query(`
      ...
      AND table_name IN ('users', 'sessions', 'audio_files', 'transcriptions', 'workouts', 'exercises', 'user_profiles', 'voice_profiles', 'speaker_verifications')
    `);
...
async function runMigration() {
  ...
  try {
    ...
    console.log(`✅ ${tableCount}/9 tables created successfully`);
...
async function initializeDatabase() {
  try {
    ...
    if (tableCount < 9) {
      console.log(`❌ Only ${tableCount}/9 required tables found. Running migration...`);
      ...
      console.log(`✅ All ${tableCount} tables already exist. Database is ready.`);
    }
  } catch (error) {
