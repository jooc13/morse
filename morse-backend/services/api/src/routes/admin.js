...
router.post('/migrate', async (req, res) => {
  // Only allow in production or with admin key
  const adminKey = req.headers['x-admin-key'];
  if (process.env.NODE_ENV === 'production' && adminKey !== 'migrate-admin-key-2024') {
  ...
  try {
    ...
    // Create indexes
    await client.query('CREATE INDEX IF NOT EXISTS idx_users_device_uuid ON users(device_uuid)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_users_email ON users(email)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_sessions_device_uuid ON sessions(device_uuid)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_sessions_status ON sessions(status)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_audio_files_session_id ON audio_files(session_id)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_audio_files_status ON audio_files(status)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_workouts_user_id ON workouts(user_id)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_workouts_device_uuid ON workouts(device_uuid)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_workouts_date_completed ON workouts(date_completed)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_exercises_workout_id ON exercises(workout_id)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_voice_profiles_device_uuid ON voice_profiles(device_uuid)');
